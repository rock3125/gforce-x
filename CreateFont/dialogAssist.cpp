...
#include "CreateFont/dialogAssist.h"
...
int AddTabItem( HWND tabControl, size_t index, const std::string& _str, long data )
{
	char* str;
	...
	item.iImage = -1;
	item.lParam = data;
	return ::SendMessage( tabControl, TCM_INSERTITEM, index, LPARAM(&item) );
};
